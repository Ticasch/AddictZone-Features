...
public class TpaCMD implements CommandExecutor {
    ...
    public TpaCMD(MainClass plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (target.getName().equals(player.getName())) {
                player.sendMessage(prefix + "Du kannst dir selbst keine Tpa senden!");
                return true;
            }
            if (new TpaToggleManager(target.getUniqueId().toString()).getToggledTpa() == true) {
                player.sendMessage(prefix + "Dieser Spieler hat seine §bTpa §7deaktiviert.");
                return true;
            }
            ...
            TextComponent cancelStart = new TextComponent(prefix + "Du hast §b" + target.getName() + " §7eine Tpa geschickt.\n" + prefix + "§bKlicke§7: [");
            ...
            if (TELEPORTINQUIRIES.containsKey(target)) {
                ...
                if (list.contains(player)) {
                    player.sendMessage(prefix + "Du hast §b" + target.getName() + " §7bereits eine Tpa geschickt.");
                } else {
                    list.add(player);
                    TELEPORTINQUIRIES.replace(target, list);
                    player.spigot().sendMessage((BaseComponent)cancelStart);
                    target.sendMessage(prefix + "§b" + target.getName() + " §7hat dir eine §bTpa §7geschickt.");
                    target.spigot().sendMessage((BaseComponent)click);
                    Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)this.plugin, () -> {
                        if (list.contains(player)) {
                            list.remove(player);
                            TELEPORTINQUIRIES.replace(target, list);
                            player.sendMessage(prefix + "§7Deine §bTpa §7an §b" + target.getName() + "§7ist abgelaufen.");
                            target.sendMessage(prefix + "§7Die §bTpa §7von §b" + player.getName() + " &7ist abgelaufen.");
                        }
                    },1200L);
                }
            } else {
                ...
                player.spigot().sendMessage((BaseComponent)cancelStart);
                target.sendMessage(prefix + "§b" + player.getName() + " §7hat dir eine §bTpa §7geschickt.");
                target.spigot().sendMessage((BaseComponent)click);
                Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)this.plugin, () -> {
                    if (list.contains(player)) {
                        list.remove(player);
            ...
            player.sendMessage(prefix + "Benutze: §b/Tpa §7<§bSpieler§7>");
        }
        return false;
    }
}
